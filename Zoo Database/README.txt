...
========================================================================
L'assegnamento prevede la realizzazione di un insieme di classi per la gestione di 
un insieme di animali di uno zoo e da una GUI come specificato nel file zoo.pdf.

Come procedere:
(1) realizzare (nel file animali.py) le classi richieste
    seguendo le indicazioni dei commenti
(2) realizzare la classe Zoo (nel file zoo.py) seguendo le indicazioni dei commenti
    per ogni metodo
(3) eseguire i test nel file main.py fornito dai docenti (senza modificarlo)
    per effettuare questo test sia main.py che testMy.py che zoo.py che animali.py
    si devono trovare nella stessa cartella
(4) se tutti i test vengono superati (Pass) e viene stampato il messaggio finale 
    che invita alla consegna, realizzare la GUI nel file gui.py
(5) nel realizzare la GUI, prima di scrivere il codice pensare all'aspetto grafico
    e alla disposizione degli elementi, eventualmente utilizzando contenitori per
    dividere lo spazio della finestra
(6) una volta realizzate tutte le operazioni richieste, inserire
    nome cognome e contatti dei componenti del gruppo nell'intestazione dei file. 
(7) consegnare inviando i file Python (ed eventuali file aggiuntivi -- 
    vedi istruzioni sotto) sul Moodle del corso alla consegna chiamata 
    "Assegnamento 3 Python Avanzato"

------------------------------------------------
Cosa contiene la cartella dell'assegnamento
------------------------------------------------

zoo2.pdf	: file contenente la descrizione dell'assegnamento

animali.py : file che contiene la definizione delle classi richieste

zoo.py 		: file che contiene la definizione delle funzioni da realizzare

gui.py		: file in cui definire la GUI con gli import necessari
...
Inserire print() temporaneamente per verificare i punti critici del codice quando il
test non sono passati.
...
da consegnare insieme al resto dei file .py.
...
La modalità di valutazione degli assegnamenti è descritta in zoo2.pdf.
